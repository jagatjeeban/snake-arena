...
- Make JSDoc useful to someone new to Snake Arena: explain where the function fits in the game or screen flow, why its behavior matters, and any lifecycle, UI-thread, renderer-capacity, deterministic-engine, fallback, or side-effect contract a caller must preserve. Describe outcomes in app terms instead of restating the function name or TypeScript signature.
...
- Inside functions, follow `src/features/game/engine/food-placement.ts`: use one blank line to separate semantic stages such as worklet directives, setup values, derived calculations, guard clauses, loops, state changes, side effects, and the final return. Keep tightly related statements, multiline expressions, object properties, JSX attributes, and short single-purpose callbacks together. Do not add a blank line after every statement or use multiple consecutive blank lines.
